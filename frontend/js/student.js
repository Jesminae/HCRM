document.addEventListener('DOMContentLoaded', () => {
    ...
    // Allow past dates by setting max to today
    document.getElementById('attDate').max = fmtTdy;
    document.getElementById('mealDate').value = fmtTmw;
    document.getElementById('mealDate').min = fmtTmw;
    ...
    document.getElementById('mealForm').addEventListener('submit', async (e) => {
        ...
        try {
            const res = await fetch('/api/student/meals/batch', {
                method: 'POST',
                headers: getAuthHeaders(),
...
async function buildMealTable() {
    ...
    try {
        const res = await fetch(`/api/student/settings?date=${date}`, {
            headers: getAuthHeaders()
        });
...
async function loadSummary() {
    ...
    try {
        const res = await fetch(`/api/student/meals/recent?month=${month}`, {
            headers: getAuthHeaders()
        });
        const data = await res.json();
...
function analyzeComplaint() {
    ...
    // Keyword heuristic representing "ML"
